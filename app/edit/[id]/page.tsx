...
import { Edit, Wand2, Palette } from "lucide-react"
...
export default async function EditPage({ params }: EditPageProps) {
    ...
    // Get the portfolio data to edit
    const generator = new PortfolioGenerator(supabase)
    const portfolio = await generator.getPortfolio(data.user.id)
    ...
    if (portfolio.userId !== data.user.id) {
        redirect("/protected")
    }
            ...
            <div className="py-8 pb-16">
                ...
                <div className="max-w-6xl mx-auto px-4 relative z-10">
                    ...
                    <div className="text-center mb-12 opacity-0 translate-y-5 animate-[fadeInUp_0.8s_ease-out_both]">
                        <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-r from-orange-600 to-red-600 rounded-full mb-6 animate-bounce">
                        ...
                        <p className="text-xl text-gray-300 mb-8 max-w-2xl mx-auto leading-relaxed">
                            Update your portfolio with new projects, skills, and information
                        </p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto mb-12">
                            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-6 transform hover:scale-105 transition-all duration-300 hover:bg-white/15">
                                ...
                                <h3 className="text-lg font-semibold text-white mb-2">Easy Updates</h3>
                                <p className="text-gray-300 text-sm">Modify your portfolio content with ease</p>
                            </div>
                            ...
                            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-6 transform hover:scale-105 transition-all duration-300 hover:bg-white/15">
                                ...
                                <h3 className="text-lg font-semibold text-white mb-2">Pre-filled Data</h3>
                                <p className="text-gray-300 text-sm">All your existing information is loaded automatically</p>
                            </div>
                            ...
                            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-6 transform hover:scale-105 transition-all duration-300 hover:bg-white/15">
                                ...
                                <p className="text-gray-300 text-sm">See changes instantly in your updated portfolio</p>
                            </div>
                        </div>
                    </div>

                    {/* Portfolio Form with existing data */}
                    <PortfolioForm userId={data.user.id} existingData={portfolio.data} portfolioId={portfolio.id} isEditing={true} />
                </div>
            </div>
        </div>
    )
